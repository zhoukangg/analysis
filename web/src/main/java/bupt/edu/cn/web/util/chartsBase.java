...
public class chartsBase {
    ...
    //3饼图
    private String pieBasic = "{title:{text:'title',left:'center'},legend:{data:[],type: 'scroll',left:'left',orient:'vertical'},tooltip:{trigger:'item',formatter:'{b}:{c}({d}%)'},color:[],series:[{name:'','radius':'55%','center': ['50%','60%'],type:'pie',data:[]}]}";
    ...
    // 5 气泡图
    private String bubbleBasic = "{\"title\":{\"text\":\"title\",\"left\":\"center\"},\"tooltip\": {\"show\": true},\"series\":[{\"type\": \"graph\", \"layout\":\"force\", \"force\": {\"repulsion\": 100, \"edgeLength\": 10}, \"label\":{\"normal\":{\"show\":true}},\"data\":[]}]}";
    // 9 箱线图
    private String boxBasic = "{title:{text:\"title\",left:\"center\"}, \"tooltip\":{show: true},\"series\":[{\"type\": \"treemap\", \"label\":{\"normal\":{\"show\":true}},\"data\":[]}]}";
    ...
    public String transDiagram(int typebefore, int typeafter, String str_option) {
        ...
        if (typebefore == 7 || typebefore == 2 || typebefore == 12 || typebefore == 4 ||
                typebefore == 5 || typebefore == 9 || typebefore == 11) {   // 折线图、面积图、散点图、雷达图、气泡图、箱线图、词云图转：
            ...
            if (typebefore == 5 || typebefore == 9 || typebefore == 11) {        // 气泡图、箱线图、词云图自动先转折线图
                JSONObject line = new JSONObject(lineBasic);
                //放标题
                line.put("title", newoption.getJSONObject("title"));
                // 取数据
                JSONArray data = newoption.getJSONArray("series").getJSONObject(0).getJSONArray("data");
                JSONArray value = new JSONArray();
                JSONArray name = new JSONArray();
                for (int i = 0; i < data.length(); i++) {
                    name.put(data.getJSONObject(i).getString("name"));
                    value.put(data.getJSONObject(i).getString("value"));
                }
                //放数据
                line.getJSONArray("series").getJSONObject(0).put("data", value);
                line.getJSONObject("xAxis").put("data", name);
                //指针转换
                newoption = line;
            ...
            } else if (typeafter == 5) {     // 气泡图
                newoption = lineToBubble(newoption);
            } else if (typeafter == 9 || typeafter == 11) {  // 箱线图、词云图
                newoption = lineToBox(newoption, typeafter);
            }
            ...
            if (typeafter == 13) {       //漏斗图
                ...
                if (typeafter == 7 || typeafter == 2) {   //折线图或面积图
                    ...
                    newoption = lineToBubble(newoption);
                } else if (typeafter == 9 || typeafter == 11) {
                    newoption = lineToBox(newoption, typeafter);
                }
            }
    ...
    private JSONObject lineToBox(JSONObject newoption, int typeafter) {     //折线图转箱线图、词云图
        JSONObject box;
        if (typeafter == 9)
            box = new JSONObject(boxBasic);
        else
            box = new JSONObject(wordCloudBasic);
        // 直线图中拿数据
        JSONArray name = newoption.getJSONObject("xAxis").getJSONArray("data");
        System.out.println("--------¬" + newoption.toString());
        JSONArray value = newoption.getJSONArray("series").getJSONObject(0).getJSONArray("data");
        // 构造箱线数据
        JSONArray boxData = new JSONArray();
        for (int i = 0; i < name.length(); i++) {
            JSONObject dataItem = new JSONObject();
            dataItem.put("name", name.getString(i));
    ...
    private JSONObject lineToBubble(JSONObject newoption) {     //折线图转气泡图

        JSONObject bubble = new JSONObject(bubbleBasic);
        // 直线图中拿数据
        JSONArray name = newoption.getJSONObject("xAxis").getJSONArray("data");
        JSONArray value = newoption.getJSONArray("series").getJSONObject(0).getJSONArray("data");
        // 构造气泡数据
        JSONArray bubbleData = new JSONArray();
        for (int i = 0; i < name.length(); i++) {
            JSONObject dataItem = new JSONObject(bubbleDataItem);
            dataItem.put("name", name.get(i));
            dataItem.put("value", value.get(i));
            JSONObject normal = dataItem.getJSONObject("itemStyle").getJSONObject("normal");
            Random random = new Random();
            int s1 = random.nextInt(255);
            int s2 = random.nextInt(255);
            int s3 = random.nextInt(255);
            String color = "rgb(" + Integer.toString(s1) + "," + Integer.toString(s2) + "," + Integer.toString(s3) + ")";
            normal.put("borderColor", color);
            normal.put("shadowColor", color);
            normal.put("color", color);
            bubbleData.put(dataItem);
        }
        bubble.getJSONArray("series").getJSONObject(0).put("data", bubbleData);
        // 标题
        String title = newoption.getJSONObject("title").getString("text");
        newoption.getJSONObject("title").put("text", title);
        // 转换指针
