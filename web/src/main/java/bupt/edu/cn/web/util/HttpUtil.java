...
/**
 * @program analysis
 * @description: ${访问数据湖中csv文件}
 * @author: kang
 * @create: 2019/06/25 23:20
 */
public class HttpUtil {

    public static final String POST_URL = "http://10.108.211.136:32111/csv/warehouse";
...
//            dataout.writeBytes(parm);
    ...
    public JSONObject filter(String str){
        try{
            JSONObject oj = JSON.parseObject(str);
            if(!oj.getString("code").equals("200")){
                return oj;
            }
            JSONArray ja = oj.getJSONArray("data");
            for (int i = 0;i<ja.size();i++) {
                JSONObject item = ja.getJSONObject(i);
                if(item.getString("dbUrl").equals("/home/jsw-data-Analysis/elt/uncleaned")
                        || item.getString("dbUrl").equals("/home/jsw-data-Analysis/elt/cleaned")){
